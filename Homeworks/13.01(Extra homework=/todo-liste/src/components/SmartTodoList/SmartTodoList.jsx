...
import Task from "../SmartTodoList/Task";


export default function SmartTodoList({task}) {
    ...
    return (
        ...
        <div>
            ...
            {
                <button style={{marginTop:'10px'}} onClick={() => {deleteAllTasks(task)}}>Delete All</button>}
            <ul>
                {
                    tasks.length == 0 ? <p>Нет активных задач</p>: 
                    tasks.map((task) => (
                        <Task key={task.id} task={task} updateTask={updateTask} deleteTask={deleteTask} deleteAllTasks ={deleteAllTasks} />
                    ))}
            </ul>
    </div>
    </div>
